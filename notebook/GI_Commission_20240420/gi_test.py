...
class GI_commission(Find_Cashbook):
    ...
    def run_delta_1(self, pattern):
        ...
        if matching_files == []:
            ...
            # rename the columns as per the working file ['TFAR', 'Policy no.', 'Pol Date', 'Insured Name', 'Comm.Recd (with GST)', 'GST amt']
            columns = ['Policy No.', 'Total', 'GST on Brokerage Commission']

            rename_col = ['Policy no.', 'Comm.Recd (with GST)', 'GST amt']
            ...
            delta['Comm.Recd (with GST)'] = pd.to_numeric(delta['Comm.Recd (with GST)'].str.replace(',',''), errors='coerce')
            delta['GST amt'] = pd.to_numeric(delta['GST amt'].str.replace(',',''), errors='coerce')
